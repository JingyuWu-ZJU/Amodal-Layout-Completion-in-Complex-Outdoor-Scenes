...
class UNet(nn.Module):
    def __init__(self, in_channels=3, w=4, n_classes=2):
        ...
        self.up1 = up(int(257 * w), int(64 * w))
        # self.up1 = up(int(262 * w), int(64 * w))
        # self.up1 = up(int(261 * w), int(64 * w))
        # self.up1 = up(int(265 * w), int(64 * w))
    ...
    def forward(self, x, category=None):
        ...
        embedding = self.label_embedding(category)
        embedding = embedding.view(b,-1,16,16)
        x6 = torch.cat((x5,embedding),dim = 1)
        x = self.up1(x6, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
